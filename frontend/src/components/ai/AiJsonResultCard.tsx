...
export function AiJsonResultCard({
  ...
  if ("suggested_priority" in payload) {
    return (
      <AdviceCard
        title={t("ai.priorityAction")}
        label={t("ai.suggestedPriority")}
        value={formatPriority(stringifyScalar(payload.suggested_priority) ?? "", t)}
        payload={payload}
      />
    );
  }
...
function ReplySuggestionCard({
  ...
  const originalSuggestion = stringifyScalar(payload.suggestion) ?? "";
  const [draft, setDraft] = useState(originalSuggestion);
  const [discarded, setDiscarded] = useState(false);
  const riskFlags = toStringList(payload.risk_flags ?? payload.riskFlags);
  const trimmedDraft = draft.trim();
  const draftPayload = {
    ...payload,
    suggestion: trimmedDraft,
    originalSuggestion,
    riskFlags,
  };
  ...
  return (
    <ResultShell title={t("ai.replySuggestion")} payload={payload}>
      <div>
        <p className="font-semibold text-ink">
          {t("ai.suggestion")}{separator(lang)}
        </p>
      ...
      {canSaveAiReply && onSaveAiReply ? (
        <div className="flex flex-wrap gap-2 pt-1">
          <Button variant="secondary" disabled={!trimmedDraft} onClick={() => onSaveAiReply(draftPayload)}>
            {t("actions.saveAiReply")}
          </Button>
          <Button variant="ghost" onClick={() => setDiscarded(true)}>
            {t("common.cancel")}
          </Button>
        </div>
      ) : null}
